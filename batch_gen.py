...
from farmagene_grapher import (
    GenomeDataManager,
    GenomicEmbedder,
    PharmacogeneGraphBuilder,
)
...
def main():
    ...
    for raw_gene_name, group in tqdm(grouped_snps, desc="Procesando Genes"):
        ...
        real_gene_name, chrom, start, end = find_gene_coordinates(
            raw_gene_name, data_manager.gtf
        )
        ...
        for _, row in group.iterrows():
            ...
            for alt in alt_alleles:
                try:
                    ...
                    variant_data = [
                        {"pos": relative_pos, "alt": alt, "type": row["variant_type"]}
                    ]
                    ...
                    errors.append(
                        f"Fallo al construir grafo {snp_id} ({alt}): {str(e)}"
                    )
    ...
    if errors:
        ...
        for err in errors[:5]:
            print(f"- {err}")
        print("(Ver 'error_log.txt' para detalles completos)")
