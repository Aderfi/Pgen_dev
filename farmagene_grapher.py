...
CHROM_MAPPING = {
    ...
    "21": "NC_000021.9",
    "chr22": "NC_000022.11",
    "22": "NC_000022.11",  # <--- ESTE ES EL QUE NECESITAS
    "chrX": "NC_000023.11",
    "X": "NC_000023.11",
    "chrY": "NC_000024.10",
    "Y": "NC_000024.10",
...
class GenomeDataManager:
    def __init__(self, fasta_path, gtf_path):
        ...
        self.gtf = pd.read_csv(
            gtf_path, sep="\t", comment="#", names=cols, low_memory=False
        )
        ...
        print(
            f"Muestra de genes extraídos: {self.gtf['gene_name'].dropna().unique()[:5]}"
        )
    ...
    def extract_gene_data(
        self, chromosome_common, start_pos, end_pos, target_gene=None
    ):
        ...
        try:
            ...
            raise ValueError(
                f"FASTA Key '{fasta_key}' no encontrada. Disponibles: {valid}"
            )
        ...
        if subset.empty:
            print(
                f"[AVISO] No se encontraron exones para {target_gene} en {chromosome_common}:{start_pos}-{end_pos}"
            )
