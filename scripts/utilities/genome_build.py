...
class GenomeGraphBuilder:
    ...
    def build_gene_graph(
        ...
        for idx, row in df_gene.iterrows():
            ...
            features = {
                ...
                "frameshift": bool(row["is_frameshift"])
                if pd.notna(row["is_frameshift"])
                else False,
    ...
    def to_pytorch_geometric(
        self, G: nx.DiGraph, patient_genotype_map: Dict[str, float]
    ):
        """
        ...
        for node, data in G.nodes(data=True):
            ...
            data["x_vtype"] = float(
                var_map.get(data.get("variant_type", "backbone"), 0)
            )
        ...
        pyg_data = from_networkx(
            G, group_node_attrs=["x_dosage", "x_type", "x_vtype", "x_score"]
        )
        return pyg_data
