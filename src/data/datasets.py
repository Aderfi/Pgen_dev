...
class DoubleTowerDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        drug_path: Union[Path, str] | None = self.drug_id_to_path.get(drug_id_df,)
        ...
        haplo_path: Union[Path, str] | None = self.gene_variant_path.get(row[self.haplo_col], None)
    ...
    def _encode_targets(self, df: pd.DataFrame, target_cols: List[str], multilabel_cols: List[str] | None = None) -> Dict[str, torch.Tensor]:
        """
        ...
        for col in target_cols:
            if col in multilabel_set:
                ...
                raw_data = df[col].fillna(pd.NA).astype(str).apply(lambda x: x.split('|') if x else [])
    ...
    try:
        ...
        try:
            ...
            if drug_data.x.shape[0] == 1 and drug_data.x.shape[1] == 5:
                print("   ⚠️ (Aviso) Se cargó el grafo vacío por defecto (checkear IDs).")
            else:
                print(f"   ✅ Grafo de fármaco cargado correctamente. Nodos: {drug_data.num_nodes}")
            ...
            print("   👉 DIAGNÓSTICO: Tu método '_build_genes_index' crea un diccionario anidado:")
            print(f"      Ej: {{'CYP2D6': {{'star4': Path(...)}} }}")
            print("      Pero en '__getitem__', intentas acceder como si fuera una ruta directa:")
            print("      'haplo_path.exists()' falla porque 'haplo_path' es un diccionario, no un Path.")
            print("   👉 SOLUCIÓN: Debes decidir si 'haplo_col' en el DF tiene 'CYP2D6' o 'CYP2D6_star4'.")
        ...
        try:
            ...
            print(f"📦 Batch generado exitosamente.")
            print(f"   Batch Size: {batch['drug_data'].batch.max() + 1}")
            print(f"   Targets Batch Shape (side_effects): {batch['targets']['side_effects'].shape}")
        except Exception as e:
            print(f"⚠️ No se pudo crear el batch (probablemente por el error anterior): {e}")
