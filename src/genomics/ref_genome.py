...
class ReferenceGenomeManager:
    ...
    def _needs_download(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"No se pudo comprobar fecha remota: {e}. Omitiendo descarga."
            )
            return False
    ...
    def index_samtools(self):
        ...
        if (
            ...
            try:
                subprocess.run(
                    f"samtools faidx {self.target_fasta}", shell=True, check=True
                )
                logger.info("\t- Índice .fai creado.")
            except subprocess.CalledProcessError:
                logger.error(
                    "XXXX -- Falló 'samtools faidx'. Asegúrate de tener samtools instalado."
                )
        else:
