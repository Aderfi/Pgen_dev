...
def smiles_to_graph_complete(smiles) -> None | Data:
    ...
    for atom in mol.GetAtoms():
        ...
        features += one_hot_encoding(atom.GetDegree(), ALLOWED_DEGREES, allow_unknown=True)
        features += one_hot_encoding(atom.GetFormalCharge(), ALLOWED_CHARGES, allow_unknown=True)
        features += one_hot_encoding(atom.GetHybridization(), ALLOWED_HYBRIDIZATIONS, allow_unknown=True)
        features += one_hot_encoding(atom.GetTotalNumHs(), ALLOWED_TOTAL_HS, allow_unknown=True)
        features += one_hot_encoding(atom.GetChiralTag(), ALLOWED_CHIRAL_TAGS, allow_unknown=False)
...
def visualizar_grafo_interpretado(grafo_pyg: Data, smiles_title: str="Molecula") -> None:
    """
    ...
    d.DrawMolecule(mol,
                   highlightAtoms=list(node_colors.keys()),
                   highlightAtomColors=node_colors)
    ...
    if isinstance(d, rdMolDraw2D.MolDraw2DSVG):
        ...
        try:
            ...
            print("Advertencia: Instala 'cairosvg' para renderizar SVG en matplotlib. Mostrando placeholder.")
            return
    else:
