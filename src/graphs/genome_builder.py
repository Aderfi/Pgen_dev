...
class GenomeGraphBuilder:
    ...
    def build_gene_graph(self, gene_name: str, output_json_path: Optional[Union[str, Path]] = None) -> Optional[nx.MultiDiGraph]:
        ...
        for pos, group in grouped_variants:
            ...
            if ref_node not in G:
                ...
                G.add_edge(split_node, ref_node, key='ref', label='reference', weight=1.0)
            ...
            for idx, row in group.iterrows():
                ...
                edge_key = f"alt_{idx}" 
                G.add_edge(split_node, var_id, key=edge_key, label='variant', weight=1.0)
        ...
        print(f"✅ Grafo completado: {G.number_of_nodes()} nodos, {G.number_of_edges()} aristas.")
        
        return G

    def build_denoised_gene_graph(self, gene_name: str, output_json_path: Optional[Union[str, Path]] = None, 
                         only_functional: bool = True) -> Optional[nx.MultiDiGraph]:
        """
        ...
        if only_functional:
            print(f"🧹 Filtrando variantes no funcionales (Ruido)...")
            ...
            print(f"📉 Reducción: {original_count} -> {len(df_gene)} variantes clínicamente relevantes.")
    ...
    def to_pytorch_geometric(self, G: nx.MultiDiGraph) -> Data:
        ...
        type_to_idx = {
            "backbone": 0, "backbone_end": 0, # Tratamos inicio/fin como backbone
            "split": 1,
            "merge": 2,
            "allele_ref": 3,
            "allele_alt": 4
