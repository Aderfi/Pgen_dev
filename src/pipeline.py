...
from src.config.manager import get_model_config, SEED, MULTI_LABEL_COLS
...
def train_pipeline(model_name: str, csv_path: str, epochs: int = 50, batch_size: int = 32):
    ...
    dims = {}
    dims["drug_feat"] = cfg.get("drug_in_features", 25)   # If you removed the duplicate line. Use 30 otherwise.
    ...
    # 2. Data Loading (Raw)
    df = DataLoaderUtils.load_dataframe(csv_path, cols=cfg["cols"], stratify_col=cfg.get("stratify_col", None))
    logger.info(f"Loaded dataframe with cols: {df.columns.tolist()} and shape: {df.shape}")
    ...
    uncertainty_net = LossFactory.create_uncertainty_wrapper(tasks=cfg["targets"], device=device)
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', patience=8)
