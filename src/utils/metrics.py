        ...
        with torch.no_grad():
            for batch in data_loader:
                ...
                if not ("drug_batch" in batch and "haplo_batch" in batch):
                    raise ValueError("El batch no contiene la estructura de grafos requerida (drug_batch, haplo_batch).")
                ...
                batch_targets = {k: v.to(self.device) for k, v in batch.get("targets", {}).items()}
