...
class TestDeepFM_PGenModel:
    ...
    def test_forward_pass(self, model_params):
        ...
        # Create dummy inputs
        inputs = {
            "gene_id": torch.randint(
                0, model_params["n_features"]["gene_id"], (batch_size,)
            ),
            "drug_id": torch.randint(
                0, model_params["n_features"]["drug_id"], (batch_size,)
            ),
        }
        ...
        # Check output shapes
        assert outputs["outcome"].shape == (
            batch_size,
            model_params["target_dims"]["outcome"],
        )
        assert outputs["side_effects"].shape == (
            batch_size,
            model_params["target_dims"]["side_effects"],
        )
