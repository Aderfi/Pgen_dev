...
def mock_dependencies():
    with (
        patch("src.pipeline.get_model_config") as mock_config,
...
def test_train_pipeline_execution(mock_dependencies):
    ...
    dummy_df = pd.DataFrame(
        {"gene_id": ["G1"] * 10, "outcome": ["0"] * 10, "_stratify": ["0"] * 10}
    )
    ...
    # Execute Pipeline
    train_pipeline(model_name="test_model", csv_path="dummy.csv", epochs=1)
