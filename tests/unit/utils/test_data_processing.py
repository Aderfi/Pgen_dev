...
class TestPGenDataProcess:
    def test_fit_transform(
        self, sample_data, feature_cols, target_cols, multi_label_cols
    ):
    ...
    def test_transform_unknown_values(self, sample_data, feature_cols, target_cols):
        ...
        new_data = pd.DataFrame(
            {"gene_id": ["G_NEW"], "drug_id": ["D1"], "target_outcome": [0]}
        )
...
class TestPGenDataset:
    def test_dataset_structure(
        ...
        assert (
            item["side_effects"].dtype == torch.float32
        )  # Multi-label usually float for BCE logic or embedding sum
