    ...
    '''
        ...
        for doc in root.findall(".//DocumentSummary"):
            ...
            if hgvs_pattern:
                ...
                if seq_pattern:
                    ref_allele = seq_pattern.group(1) + "?" # Marcamos con ? por incertidumbre
            ...
            variant_seq = re.search(r'SEQ=\[[ACGT/]+\]', docsum_text)[0] if re.search(r'SEQ=\[[ACGT/]+\]\|', docsum_text) else "N/A"
            variant_len = re.search(r'LEN=\d+', docsum_text)[0] if re.search(r'LEN=\d+', docsum_text) else "N/A"

            variant_len = variant_len.replace('LEN=', '') if variant_len != "N/A" else variant_len
            variant_seq = variant_seq.replace('SEQ=[', '').replace(']', '') if variant_seq != "N/A" else variant_seq
        ...
        "NC_000021": 9,  "NC_000022": 11, "NC_000023": 11, # X
        "NC_000024": 10  # Y
    }
            ...
            if not unique_alts and ref_base:
                unique_alts = [alt_allele.replace('?', '')] if alt_allele != "N/A" else []
            variant_constructed = f"{ref_base}>{'|'.join(unique_alts)}" if ref_base else "N/A"
            ...
            if variant_len.isdigit() and int(variant_len) > 1:
                ...
                spdi_match = re.findall(r"(NC_\d+\.\d+):(\d+):([ACGT]+):([ACGT]+)", espid)
    ...
    try:
        ...
        for doc in root.findall(".//DocumentSummary"):
            ...
            if variant_len_int > 1:
                ...
                spdi_matches = re.findall(r"(NC_\d+\.\d+):(\d+):([ACGT]*):([ACGT]*)", espid)
                ...
                if spdi_matches:
                    for spdi_chrom, spdi_pos, spdi_del, spdi_ins in spdi_matches:
                        ...
                        # Filtrar solo GRCh38
                        if base_id in grch38_versions and int(version_str) == grch38_versions[base_id]:
                            
                            # Validar con HGVS en DOCSUM para obtener el "Tipo" (del, ins, dup)
                            # Buscamos el cromosoma exacto + patrón g.
                            pattern_hgvs = re.escape(spdi_chrom) + r":g\.([\d_]+)([a-z]+)"
                            ...
                            if type_match:
                                ...
                                if (vcf_ref == vcf_alt[:len(vcf_ref)]) and len(vcf_alt) > len(vcf_ref) and (vcf_ref in vcf_alt):
            ...
            if not processed_complex:
                ...
                for full_id, version_str, ref, alt in matches_snv:
                    ...
                    if b_id in grch38_versions and grch38_versions[b_id] == ver:
                        if ref_base is None: ref_base = ref
                        valid_variants.append(alt)
...
def get_snp_data_batch_ULTRA(rs_list):
    ...
    try:
        ...
        for doc in root.findall(".//DocumentSummary"):
            ...
            variant_len_str = "0"
            len_match = re.search(r'LEN=(\d+)', docsum_text)
            if len_match: variant_len_str = len_match.group(1)
            variant_len_int = int(variant_len_str)
                ...
                if spdi_matches:
                    for spdi_chrom, spdi_pos, spdi_del, spdi_ins in spdi_matches:
                        ...
                        if base_id in grch38_versions and int(version_str) == grch38_versions[base_id]:
                            ...
                            pattern_indel = re.escape(spdi_chrom) + r":g\.(\d+)(?:_(\d+))?([a-z]+)"
                            ...
                            pattern_str = re.escape(spdi_chrom) + r":g\.(\d+)([ACGT]*)\[(\d+)\]"
                            ...
                            if match_str:
                                ...
                                end_pos = match_indel.group(2) # Puede ser None si es un punto único
                                variant_type_found = match_indel.group(3) # del, ins, dup
            ...
            if not processed_complex:
                ...
                chrpos = doc.findtext("CHRPOS")
                chr_val, pos_val = chrpos.split(':') if chrpos and ':' in chrpos else (doc.findtext("CHRONO"), "N/A")
                ...
                if ref_simple == "N/A":
                     ...
                     if seq_pat: ref_simple, alt_simple = seq_pat.group(1), seq_pat.group(2)
                ...
                for full_id, ver, r, a in matches_snv:
                    ...
                    if base in grch38_versions and int(ver) == grch38_versions[base]:
                        if ref_base is None: ref_base = r
                        valid_alts.append(a)
                    ...
                    for spdi_chrom, spdi_pos, spdi_del, spdi_ins in spdi_matches:
                        ...
                        if base_id in grch38_versions and int(version_str) == grch38_versions[base_id]:
                            ...
                            if match_str:
                                ...
                                hgvs_pos_str = f"{start_pos}_{end_pos}" if end_pos else start_pos
                            ...
                            norm_chrom = (spdi_chrom.split('.')[0]).replace("NC_00000", "").replace("NC_0000", "")
                            if norm_chrom == "23": 
                                norm_chrom = "X"
                            elif norm_chrom == "24": 
                                norm_chrom = "Y"
