...
tarkov = df_filtered[["phenotype category", "direction of effect", "effect_function", "phenotype_product"]].copy()


for col in tarkov:
    print(f"\n {col}:")
    if tarkov[col].unique().size < 35:
        print("\t -- Val Count \n\t", tarkov[col].value_counts())  # Número de valores únicos
    else:
        print("\t -- Número de valores únicos:\n\t", tarkov[col].nunique())
    
    print("\t -- NaN count:\n\t", tarkov[col].isna().sum())  # Número de NaN

    print("PORCENTAJE DE CELDAS VACIAS EN LA COLUMNA:  ", tarkov[col].isna().sum() / len(tarkov))
