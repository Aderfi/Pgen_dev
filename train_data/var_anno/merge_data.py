...
def process_genomic_data():
    ...
    for key, filepath in files.items():
        if os.path.exists(filepath):
            try:
                ...
                dfs[key] = df
                print(
                    f"✅ {key.upper()} cargado: {df.shape[0]} filas, {df.shape[1]} columnas"
                )
    ...
    if not ID_COLS:
        print(
            "❌ Error: No hay columnas comunes entre los 3 archivos para usar como llave (ID)."
        )
        return

    # --- 3. FUNCIÓN DE FUSIÓN INTELIGENTE ---
    def merge_and_consolidate(
        df_left, df_right, keys, suffix_left="_old", suffix_right="_new"
    ):
        """
        ...
        for col_old in cols_to_fix:
            base_name = col_old[
                : -len(suffix_left)
            ]  # Nombre original (ej: 'Description')
        ...
        if count_fused > 0:
            print(
                f"   ↳ Se fusionaron (concatenaron) {count_fused} columnas con nombres repetidos."
            )
