...
NAN_TOKENS = set(['__UNK__', '__UNDETERMINED__', '__NOTSTATED__', 'nan', 'NaN', '', ' ', 'NA', 'N/A'])
...
def map_value(x):
    ...
    category_list_return = [cat.strip().replace(r"\s+", "_") for cat in category_list if cat]
...
df["effect_function"] = df["effect_function"].str.replace(
    r'\b\s*(?:to|of)\b', 
    '', 
    regex=True, 
    flags=re.IGNORECASE
...
df["phenotype category"] = df["phenotype category"].str.replace(r'\s*,\s*', '|', regex=True)
...
df["population_phenotypes_or_diseases"] = df["population_phenotypes_or_diseases"].apply(clean_phenotypes_product)
