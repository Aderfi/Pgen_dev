...
def plot_molecule_reconstructed(data: Data, title: str = None,
                                 save_path: str = None, figsize=(20, 9)):
    """
    ...
    fig, (ax_mol, ax_pyg) = plt.subplots(1, 2, figsize=figsize,
    ...
    ax_mol.set_title("2D Structure  (RDKit)", fontsize=11, fontweight="bold", pad=8, color="#333")

    # Leyenda de átomos presentes
    if data.x is not None:
        present_atoms = {round(f[0] * 100) for f in data.x.tolist()}
        atom_legend = [
            mpatches.Patch(color=ATOM_COLORS.get(a, DEFAULT_ATOM_COLOR),
                           label=ATOMIC_SYMBOLS.get(a, f"Z={a}"))
            for a in sorted(present_atoms) if a in ATOM_COLORS
        ]
        ax_mol.legend(handles=atom_legend, loc="lower left",
                      fontsize=8, framealpha=0.85, title="Atoms", title_fontsize=8)
    ...
    for src, dst, attrs in G.edges(data=True):
        btype = attrs.get("bond_type", 0)
        nx.draw_networkx_edges(G, pos, edgelist=[(src, dst)], ax=ax_pyg,
                               edge_color=[attrs.get("color", "#888888")],
                               width=bond_widths.get(btype, 1.5), alpha=0.82)

    nx.draw_networkx_nodes(G, pos, ax=ax_pyg, node_color=node_colors,
                           node_size=node_sizes, alpha=0.95,
                           linewidths=1.0, edgecolors="#333333")

    labels = {n: ATOMIC_SYMBOLS.get(G.nodes[n].get("atomic_num", 6), "?") for n in G.nodes()}
    nx.draw_networkx_labels(G, pos, labels, ax=ax_pyg,
    ...
    ax_pyg.legend(handles=bond_legend, loc="lower left",
                  fontsize=8, framealpha=0.85, title="Bond type", title_fontsize=8)

    ax_pyg.set_title("PyG Graph  (atoms as nodes)", fontsize=11, fontweight="bold", pad=8, color="#333")
    ax_pyg.set_axis_off()
